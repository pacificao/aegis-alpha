...
def test_fully_qualified_entry_is_planned_without_risk_or_broker_call():
    ...
    with SessionLocal() as db:
        ...
        spec={"universe":{"symbols":[symbol],"exclude_symbols":[],"asset_types":["EQUITY"]},"entry_rules":[{"field":"event_yield_pct","operator":"gte","value":0.1,"reason":"YIELD"}],"exit_rules":[{"field":"recovered","operator":"eq","value":True,"reason":"RECOVERED"}],"filters":[],"parameters":{"entry_days_before_ex_date":1},"position_sizing":{"max_position_pct":1,"max_strategy_allocation_pct":100}};version=StrategyVersion(scenario_id=scenario.id,version=version_number,specification=spec,checksum=marker.ljust(64,"0")[:64],created_by="test");db.add(version);db.flush()
        ...
        created=create_qualified_plans(db,today);assert len(created)==1;plan=db.get(PlannedTrade,created[0]);assert plan.symbol==symbol and plan.planned_entry_date==entry and plan.reserved_notional==2 and plan.created_by=="system:qualified-planner" and plan.final_risk_assessment_id is None
