...
- Migration `0013_single_broker_account` records only the pseudonymous selection. The UI exposes only `SINGLE_ACCOUNT` or `NOT_SELECTED`, never the reference or account number. Existing multi-account Aegis snapshots are scheduled for removal after migration and before a clean selected-account resynchronization. Trading remains `DISABLED`.
