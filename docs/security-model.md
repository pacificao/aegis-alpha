...
- Live trading is disabled by default at both Aegis and the isolated gateway. When separately operator-enabled, submission additionally requires a short-lived database authorization, exact immutable human approval, fresh deterministic RiskEngine authorization, clear kill switch and circuit breaker, a single selected account, a $1 controlled-live ceiling, idempotent intent locking, and intended-versus-actual reconciliation. Ambiguous outcomes engage the circuit breaker.
