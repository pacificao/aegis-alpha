...
Recommended initial staging size is a 2 vCPU/4 GiB Ubuntu LTS Droplet; production begins at 4 vCPU/8 GiB after measured load tests, with separate staging and production projects/VPCs. Cloud Firewall inbound policy is TCP 443 from anywhere, TCP 80 only for redirect/ACME, and the verified SSH port only from trusted administration CIDRs; deny all other inbound traffic. Databases use private VPC endpoints/security groups only. DNS uses separate staging and production names, CAA records, short migration TTLs, then managed TLS renewal monitoring.
