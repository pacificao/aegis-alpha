...
Installation requires sudo and must be performed by Nathan. Use Docker's current official Ubuntu repository instructions, not an unreviewed convenience script. After repository setup, install `docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin`, add `nathan` to the `docker` group only if accepted, log out/in, and verify:
