...
def execute(db:Session,client:BrokerGatewayClient,settings,intent_id:int,actor:str)->dict:
    ...
    if result.get("status")=="REJECTED":record.status="REJECTED";row.status="BROKER_REJECTED";record.reconciliation={"status":"REJECTED","requires_human_attention":False,"broker_evidence":result.get("broker_evidence",{})};db.commit();return {"id":record.id,"intent_id":row.id,"status":record.status,"order_placed":False,"reconciliation":record.reconciliation,"idempotent":False}
