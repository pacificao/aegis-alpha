...
def reconcile(intended:dict,actual:dict,fills:list[dict])->dict:
    ...
    return {"status":"MATCHED" if matched else "MISMATCH","field_checks":checks,"fill_quantity":fill_quantity,"fill_quantity_valid":fill_valid,"intended_checksum":canonical_checksum(expected),"actual_checksum":canonical_checksum(observed),"requires_human_attention":not matched}
