...
def evaluate(policy:dict[str,Any],proposal:dict[str,Any],controls:dict[str,bool],now:datetime|None=None)->dict:
    ...
    c=[
      ...
      _check("DAILY_LOSS",float(proposal["daily_pnl_pct"])>=-policy["max_daily_loss_pct"],proposal["daily_pnl_pct"],-policy["max_daily_loss_pct"],"Daily loss boundary"),
      _check("DRAWDOWN",float(proposal["drawdown_pct"])<=policy["max_drawdown_pct"],proposal["drawdown_pct"],policy["max_drawdown_pct"],"Portfolio drawdown boundary"),
      _check("VOLATILITY",float(proposal["annualized_volatility_pct"])<=policy["max_annualized_volatility_pct"],proposal["annualized_volatility_pct"],policy["max_annualized_volatility_pct"],"Annualized volatility boundary"),
